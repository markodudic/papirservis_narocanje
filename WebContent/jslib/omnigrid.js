...
var omniGrid = new Class({
	...
	loadData: function (url)
	{
		...
		param.sortorder = this.options.sortOrder;
		param.seltext = this.options.selText;
		param.selvozilo = this.options.selVozilo;
		param.selstatus = this.options.selStatus;
		param.datumod = this.options.datumOd;
		param.datumdo = this.options.datumDo;
		...
		if (this.options.data && this.options.data.length)
		{
			...
			for (var r=0; r<rowCount; r++)
			{
				...
				for (var c=0; c<columnCount; c++)
				{
					...
					div.setAttribute('name', columnModel.dataIndex);
	...
	draw: function(){	
		...
		for (var c = 0; c < columnCount; c++) {
			...
			div.set('id', columnModel.dataIndex);
