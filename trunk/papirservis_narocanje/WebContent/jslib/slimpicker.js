...
var SlimPicker = new Class({
	...
	options: {
		...
		autoHide: false,                   // Without this, it won't set a timer to hide the calendar whenever you move away.
		...
		yearStart: (new Date().getFullYear() - 85),                     // Default starting year for dropdown options is 5 years ago
	...
	setCurrentDate: function() {
		...
		if (inputValue != '') {
			...
			this.current = Date.parse(inputValue);
		} else {
...
//					this.moveFocus(e.shift);
	...
	calendarClick: function(e) {
		...
		// BSF za prazen box
		else if ($(e.target).getParent('.sp_cal table tbody')) { // BSF za prazen box
			this.input.value = '';
		}
	},
